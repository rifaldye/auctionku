      ...
      <div class="ps-section__content pb-50">
        <div class="masonry-wrapper" data-col-md="4" data-col-sm="2" data-col-xs="1" data-gap="30" data-radio="100%">
          <div class="ps-masonry">
            ...
            @if($row->produk->end_at > $nows)
            <div class="grid-item">
              <div class="grid-item__content-wrapper">
                <div class="ps-shoe mb-30">
                  <div class="ps-shoe__thumbnail">
                    <img src="{{asset('images/gambarproduk/'.$row->produk->gambar->first()->gambar)}}" width="300px" height="200px" alt=""><a class="ps-shoe__overlay"  href="{{route('produk',['toko'=>$row->produk->toko->id,'slug'=>$row->produk->slug])}}"></a>
                  </div>
                  <div class="ps-shoe__content">
                    <div class="ps-shoe__variants">

                    </div>
                    <div class="ps-shoe__detail"><a class="ps-shoe__name" href="#">{{$row->produk->judul}}</a>
                      <p class="ps-shoe__categories"><a href="{{route('produk',['toko'=>$row->produk->toko->id,'slug'=>$row->produk->slug])}}">{{$row->produk->kategori->nama}}</a></p><span class="ps-shoe__price">
                        <p style="margin:0px;">Last bid</p> Rp {{number_format($row->produk->bid->sortByDesc('id')->first()->nominal)}} </span>
                    </div>
                  </div>
                </div>
                <div class="col-xs-12" style="margin-top:-30px; margin-left:-18px;">
                  @if( $nows->diffInDays($row->produk->end_at) == 0)
                  <p>Waktu Tersisa <span>{{$nows->diffInHours($row->produk->end_at)}} Jam lagi</span> </p>
                  @else
                  <p>Waktu Tersisa <span>{{$nows->diffInDays($row->produk->end_at)}} Hari lagi</span> </p>
                  @endif
                  <div class="progress" style="height:5px; background-color:#e0e0e0;">
                    @if( $nows->diffInDays($row->produk->end_at) == 0)
                    <div class="progress-bar bg-danger" role="progressbar" style="width:{{100 - ($nows->diffInHours($row->produk->end_at)/24*10)}}%;" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100"></div>
                    @else
                    <div class="progress-bar bg-info" role="progressbar" style="width: {{10-$nows->diffInDays($row->produk->end_at)}}0%;" aria-valuenow="50" aria-valuemin="0" aria-valuemax="100"></div>
                    @endif
                  </div>
                </div>
              </div>
